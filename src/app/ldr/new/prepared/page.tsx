...
interface WasteLineEntryState {
   ...
   * description -- helps confirm/choose the right subcategory. Not part of
   * LdrWasteLineEntry; nothing to store, the manifest already has it. */
...
function NewLdrNoticePageInner() {
  ...
  // The generator and receiving facility are no longer picked directly --
  // they're derived from whichever manifest the user selects below, same
  // reasoning as this app's other "select the anchor record first, reuse
  // it everywhere downstream" flows (waste profiles, label printing).
  const [manifest, setManifest] = useState<ManifestSummary | null>(null);
  const [mtnInput, setMtnInput] = useState("");
  const [isLookingUp, setIsLookingUp] = useState(false);
  const [lookupError, setLookupError] = useState<string | null>(null);
  const [recentManifests, setRecentManifests] = useState<RecentManifestSearch[]>([]);

  const [preparedByName, setPreparedByName] = useState("");
  const [entries, setEntries] = useState<WasteLineEntryState[]>([]);
  ...
  useEffect(() => {
    listRecentManifestSearchesAction().then(setRecentManifests);
  }, []);

  /**
   * Pulls a manifest's own waste lines to determine the LDR's waste codes,
   * instead of the generator/facility/codes being typed in by hand --
   * works whether the manifest is still an unsigned saved draft or has
   * already been signed, since EPA assigns a real MTN (and the lines
   * become fetchable) as soon as it's first saved, well before every
   * party has signed. One entry per actual manifest waste line that
   * carries an EPA hazardous waste code (not aggregated); howManaged
   * always starts at "A" -- there's no way to derive it from manifest
   * data, and it's the overwhelmingly common real answer anyway.
   */
  const loadManifest = (mtn: string) => {
    const trimmed = mtn.trim();
    if (!trimmed) return;
    setIsLookingUp(true);
    setLookupError(null);
    Promise.all([refetchManifestAction(trimmed), getWasteLineMetadataAction(trimmed)]).then(
      ([result, metadataByLine]) => {
        ...
        setManifest({
          epaMtn: m.manifestTrackingNumber,
          generatorEpaSiteId: m.generator.epaSiteId,
          generatorName: m.generator.name,
          receivingFacilityEpaSiteId: m.designatedFacility.epaSiteId,
          receivingFacilityName: m.designatedFacility.name,
          status: m.status,
        });
  ...
  // Backward-compatible deep link (e.g. a "Prepare LDR notice" link
  // elsewhere in the app that already knows the MTN) -- runs the same
  // lookup as the manual picker below, just automatically on load.
  const searchParams = useSearchParams();
  const deepLinkMtn = searchParams.get("mtn");
  useEffect(() => {
    if (deepLinkMtn) {
      setMtnInput(deepLinkMtn);
      loadManifest(deepLinkMtn);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps -- one-time prefill on mount
  }, [deepLinkMtn]);
  ...
  // §5) -- runs whenever a manifest is loaded and its lines carry codes, so
  // the user finds out a new notice may not even be needed before filling
  // out the rest of the form.
  useEffect(() => {
    const wasteLines = toLdrWasteLines(entries);
    if (!manifest || wasteLines.length === 0) {
      setExistingNotice(null);
      setCheckedForExisting(false);
      return;
    }
    let cancelled = false;
    findActiveLdrNoticeAction(manifest.generatorEpaSiteId, manifest.receivingFacilityEpaSiteId, wasteLines).then(
      (notice) => {
        if (cancelled) return;
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- entries is a plain array rebuilt each render; comparing its derived codes is what actually matters
  ...
  return (
    <div style={{ maxWidth: "760px", margin: "40px auto", fontFamily: "sans-serif", padding: "0 16px" }}>
      ...
      <p style={{ color: "#666" }}>
        ...
        receiving facility. Select the manifest this shipment is on below — its own waste lines
        determine the EPA hazardous waste codes, generator, and receiving facility, whether that
        manifest is still a saved draft or has already been signed. New to LDR?{" "}
      ...
      {!manifest ? (
        <div style={{ border: "1px solid #ddd", borderRadius: "6px", padding: "14px", marginTop: "16px" }}>
          <label style={{ display: "block", marginBottom: "5px", fontSize: "14px", fontWeight: 600, color: brand.navy }}>
            Select a manifest
          </label>

          {recentManifests.length > 0 && (
            <div style={{ marginBottom: "10px" }}>
              <p style={{ margin: "0 0 6px", fontSize: "12px", color: "#888" }}>Your manifests</p>
              <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                {recentManifests.map((m) => (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      setMtnInput(m.epaMtn);
                      loadManifest(m.epaMtn);
                    }}
                    style={{
                      textAlign: "left",
                      background: "white",
                  ...
                  >
                    <strong style={{ color: brand.navy }}>{m.epaMtn}</strong>{" "}
          ...
          <div style={{ display: "flex", gap: "8px" }}>
            <input
              ...
              placeholder="Manifest tracking number (MTN)"
              style={{ ...inputStyle, flex: 1 }}
            />
            <button
              type="button"
              onClick={() => loadManifest(mtnInput)}
              disabled={isLookingUp || !mtnInput.trim()}
              style={primaryButtonStyle(isLookingUp || !mtnInput.trim())}
            >
              {isLookingUp ? "Looking up…" : "Look up"}
            </button>
          </div>
          {lookupError && <p style={{ color: "#c0392b", fontSize: "13px", marginTop: "6px" }}>❌ {lookupError}</p>}
        </div>
      ) : (
        <div
          style={{
        ...
        >
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "10px" }}>
            <div>
              <p style={{ margin: 0, fontWeight: 600, color: brand.navy }}>
                Manifest {manifest.epaMtn}
                {manifest.status ? ` — ${manifest.status}` : ""}
              </p>
              <p style={{ margin: "4px 0 0", color: "#333" }}>
                Generator: {manifest.generatorName} ({manifest.generatorEpaSiteId})
              </p>
              <p style={{ margin: "2px 0 0", color: "#333" }}>
                Receiving facility: {manifest.receivingFacilityName} ({manifest.receivingFacilityEpaSiteId})
              </p>
            </div>
            <button
              type="button"
              onClick={changeManifest}
              style={{ background: "none", border: "none", color: brand.blue, cursor: "pointer", fontSize: "12px", whiteSpace: "nowrap" }}
            >
              Change manifest
            </button>
          </div>
      ...
      {manifest && checkedForExisting && existingNotice && (
        <div
          style={{
            background: "#fff8e6",
      ...
      {manifest && (
        <form action={formAction} style={{ display: "flex", flexDirection: "column", gap: "14px", marginTop: "16px" }}>
          <input type="hidden" name="generatorEpaSiteId" value={manifest.generatorEpaSiteId} />
          <input type="hidden" name="receivingFacilityEpaSiteId" value={manifest.receivingFacilityEpaSiteId} />
          <input type="hidden" name="receivingFacilityName" value={manifest.receivingFacilityName} />
          <input type="hidden" name="epaMtn" value={manifest.epaMtn} />
          ...
          <div>
            ...
            {entries.length === 0 && (
              <p style={{ fontSize: "13px", color: "#888" }}>
                This manifest has no lines carrying an EPA hazardous waste code — an LDR notice
                isn&apos;t applicable.
              </p>
            )}
            {entries.map((entry) => (
              ...
              >
                ...
                >
                  Remove this waste line — not covered by this notice
                </button>
              </div>
            ))}
          </div>
          ...
          <button
            type="submit"
            disabled={isPending || blockedByExisting || entries.length === 0}
            style={primaryButtonStyle(isPending || blockedByExisting || entries.length === 0)}
          >
