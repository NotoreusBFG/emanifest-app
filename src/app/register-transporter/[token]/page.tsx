 ...
 * protectedPaths list. A transporter taps an invite link and registers
 * their own RCRAInfo credentials with no ManifestMate login — see
 * src/app/actions/transporterRegistrationActions.ts for the security model
 * (token-gated SECURITY DEFINER functions, not RLS row visibility).
 */
export default async function TransporterRegistrationPage({ params }: { params: Promise<{ token: string }> }) {
