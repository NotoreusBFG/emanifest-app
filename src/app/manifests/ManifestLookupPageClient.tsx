...
interface ManifestLookupPageClientProps {
   ...
   * server-side in page.tsx (a client component can't read it directly —
   * getFeatureFlag needs a server Supabase client). When off, the "By
   * generator" toggle doesn't render at all and the page behaves exactly as
   * it did before this feature existed, rather than showing a broken/dead
   * toggle. */
  generatorSearchEnabled: boolean;
}

// useSearchParams() (for the ?mtn= deep link from /dashboard) requires a
// Suspense boundary in the App Router, or the build fails.
export function ManifestLookupPageClient({ generatorSearchEnabled }: ManifestLookupPageClientProps) {
  return (
    <Suspense fallback={null}>
      <ManifestLookupPageInner generatorSearchEnabled={generatorSearchEnabled} />
    </Suspense>
  );
}

function ManifestLookupPageInner({ generatorSearchEnabled }: ManifestLookupPageClientProps) {
  ...
  // "By generator" mode — plain useState toggle, same pattern as
  // login/page.tsx's signin/signup switch (no dedicated Tabs component in
  // this app). Kept entirely separate from the MTN-lookup state above.
  // Forced back to "mtn" whenever the flag is off, in case it was ever set
  // to "generator" while the flag was on and then got turned off mid-session.
  const [lookupMode, setLookupMode] = useState<"mtn" | "generator">("mtn");
  const effectiveMode = generatorSearchEnabled ? lookupMode : "mtn";
  const [resolvedGenerator, setResolvedGenerator] = useState<ResolvedGenerator | null>(null);
  ...
  return (
    <div style={{ maxWidth: "600px", margin: "40px auto", fontFamily: "sans-serif" }}>
      ...
      {generatorSearchEnabled && (
        <p className="no-print" style={{ display: "flex", gap: "16px", fontSize: "14px", marginBottom: "12px" }}>
          <button
            type="button"
            onClick={() => setLookupMode("mtn")}
            style={{
              background: "none",
              border: "none",
              padding: 0,
              cursor: "pointer",
              fontWeight: effectiveMode === "mtn" ? 700 : 400,
              color: effectiveMode === "mtn" ? brand.navy : brand.blue,
              textDecoration: effectiveMode === "mtn" ? "none" : "underline",
            }}
          >
            By tracking number
          </button>
          <button
            type="button"
            onClick={() => setLookupMode("generator")}
            style={{
              background: "none",
              border: "none",
              padding: 0,
              cursor: "pointer",
              fontWeight: effectiveMode === "generator" ? 700 : 400,
              color: effectiveMode === "generator" ? brand.navy : brand.blue,
              textDecoration: effectiveMode === "generator" ? "none" : "underline",
            }}
          >
            By generator
          </button>
      ...
      {effectiveMode === "generator" && (
        <>
          <p className="no-print" style={{ color: "#666" }}>
            Search all of EPA&apos;s live manifest records for a generator, by name or EPA ID — not
            just manifests you&apos;ve looked up in ManifestMate before.
          </p>

          <div className="no-print">
            <GeneratorSearchPanel onResolved={setResolvedGenerator} />
          </div>

          {resolvedGenerator && <GeneratorManifestResults resolved={resolvedGenerator} />}
        </>
      )}
    </div>
  );
  ...
  return (
    <Card className="p-5">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "12px" }}>
        ...
        <SendForSignature mtn={manifest.manifestTrackingNumber} />
      </div>
      <p>
