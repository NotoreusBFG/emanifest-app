...
import { getTeamInviteStatusAction } from "@/app/actions/teamActions";
...
export default async function AcceptTeamInvitePage({
  ...
  const status = await getTeamInviteStatusAction(token);
  ...
  return (
    <Shell>
      <p>
        <strong>{status.ownerEmail}</strong> has invited you to join their ManifestMate team — you&apos;ll
        be able to create and sign manifests, save waste profiles, and print labels for their
        generator sites, all using their EPA credentials. You won&apos;t need your own RCRAInfo
        registration.
      </p>
      <p style={{ color: "#666", fontSize: "14px" }}>
        Everything you do this way is recorded against {status.ownerEmail}&apos;s account, and shows
        up in their dashboard alongside anything they do themselves. You can only be on one team at
        a time.
      </p>
...
function Shell({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ maxWidth: "480px", margin: "60px auto", fontFamily: "sans-serif" }}>
      <h1 style={{ color: brand.navy, fontSize: "22px" }}>Team invite</h1>
